...
std::vector <int> ILP_Solver::find_optimal_path_BnB() {
    ...
    std::priority_queue<std::pair<std::vector<double>, std::vector<double>>> q;
    std::vector<double> best_solution;
    double best_solution_value = std::numeric_limits<double>::min();

    // add initial problem to queue
    q.push(std::make_pair(b_vec_, c_vec_));
    ...
    while (!q.empty()) {
        ...
        Simplex simplex(a_mat_, node.first, c_vec_);
        std::vector<double> solution = simplex.solve();  
        if (solution.empty()) {
            std::cout << "Simplex solver couldn't find feasible solution" << std::endl;
            continue;
        }
        double solution_value = compute_value_gained(solution);
        std::cout << "Current value: " << solution_value << ", Best value: " << best_solution_value << std::endl;
        ...
        if (is_integer_solution(solution)) { 
            ...
            for (int i = 0; i < solution.size(); ++i) {
                if (!is_integer(solution[i])) { 
                    ...
                    auto new_node_0 = modify_constraint(i, 0, node.first, node.second);
                    auto new_node_1 = modify_constraint(i, 1, node.first, node.second);
                    q.push(new_node_0);
                    q.push(new_node_1);
                    break;
                }
    ...
    if (best_solution.empty()) {
        ...
        std::cout << "Reward gained: "<< compute_value_gained(best_solution) << std::endl;
        // RCLCPP_INFO(rclcpp::get_logger("rclcpp"),""("Reward gained: "<< gain).c_str()"");
...
void ILP_Solver::prepare_data_simplex() {
        ...
        for (int i; i < n_nodes_; ++i) {
            for (int j; j < n_nodes_; ++j) {
                if (i != j) {
        ...
        a_mat_.push_back(flatten(dist_matrix_));
...
void ILP_Solver::add_slack_variables_simplex() {
    ...
    if (use_big_M_) {
        ...
        c_vec_[idx] = M_;
        // for end node constraint
        idx = (n_nodes_ * n_nodes_ ) + (n_nodes_ * n_nodes_) + 1;
        c_vec_[idx] = M_;
    }
}

void ILP_Solver::combinations_recursive(std::vector<int>& options, int k, std::vector<int>& combination, int start, std::vector<std::vector<int>>& result) {
    if (k == 0) {
...
double ILP_Solver::compute_value_gained(const std::vector<double>& best_solution) {
    double gain = 0.0;
...
std::pair<std::vector<double>, std::vector<double>> ILP_Solver::modify_constraint(int i, int new_value, const std::vector<double>& b_vec, const std::vector<double>& c_vec) {
    std::vector<double> new_b_vec = b_vec;
    std::vector<double> new_c_vec = c_vec;
    // modify constraint for x_i
    new_b_vec[i] = new_value;
    // add big M
    if (use_big_M_) {
        new_c_vec[n_nodes_*n_nodes_ + i] = M_;
    }
    return std::make_pair(new_b_vec, new_c_vec);
}

std::vector<int> ILP_Solver::reconstruct_path(const std::vector<double>& solution) {
    std::vector<int> optimal_path;
    std::vector<std::vector<double>> solution_matrix = reshape(solution, n_nodes_, n_nodes_);
    int current_node = 0;
    int end_idx = n_nodes_ -1;
...
ILP_Solver::ILP_Solver(const std::vector<double>& rewards, std::vector<std::vector<double>>& dist_matrix, const double& dist_max) {
    dist_max_ = dist_max;
    rewards_ = rewards;
